...
resource "boundary_target" "linux_admin" {
  ...
  storage_bucket_id        = var.session_recording_bucket_id
}

resource "boundary_alias_target" "ssh_target_alias" {
  name                      = "ssh_target_alias"
  description               = "Alias to target SSH using host ssh.boundary"
  scope_id                  = "global"
  value                     = "ssh.boundary"
  destination_id            = boundary_target.linux_admin.id
  authorize_session_host_id = boundary_host_static.linux_servers.id
}
...
#   scope_id    = var.boundary_org_id
#   plugin_name = "aws"
#   bucket_name = var.session_recording_bucket
